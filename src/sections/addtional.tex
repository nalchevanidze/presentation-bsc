...
\begin{enumerate}
  \li{Scalars, Wrappers, Interfaces} Derive scalar, wrapper, interface if the type has explicitly specified associated kinds. 
  \li{Field Arguments} Derive field arguments if the type is used as an argument of the object field. 
  \li{Enums} Derive enum if all data type constructors are empty.
  \li{Objects} Derive object if the type has a single non-empty constructor, and its parent is Object or Union. 
  \li{Input objects} Derive input object if the type has a single non-empty constructor, and its parent is input object or Field Arguments.
  \li{Unions} Derive Union if the type has multiple constructors and its parent is Object or Union.
